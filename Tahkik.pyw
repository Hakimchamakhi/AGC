...
def search():
	q2 = q.get()
	query = "SELECT num, nom_client, nom_adv, num_ibt, tribunal, accu, bureau_thk, result, commentaire FROM tbl_tahkik WHERE num LIKE '%"+q2+"%' OR nom_client LIKE '%"+q2+"%' OR nom_adv LIKE '%"+q2+"%' OR num_ibt LIKE '%"+q2+"%' OR tribunal LIKE '%"+q2+"%' OR bureau_thk LIKE '%"+q2+"%' OR result LIKE '%"+q2+"%' OR accu LIKE '%"+q2+"%' "
	cursor.execute(query)
	rows = cursor.fetchall()
	update(rows)

def clear():
	query="SELECT num, nom_client, nom_adv, num_ibt, tribunal, accu, bureau_thk, result, commentaire FROM tbl_tahkik ORDER BY num DESC"
	cursor.execute(query)
	rows = cursor.fetchall()
	update(rows)
...
def update_customer():
	...
	if messagebox.askyesno("Confirmation", "Confirmation de mettre à jour client"):
		query = "UPDATE tbl_tahkik SET  num = %s, nom_client = %s, nom_adv = %s, num_ibt = %s, tribunal = %s, accu = %s, bureau_thk = %s, result = %s, commentaire = %s where num= %s"
		cursor.execute(query,( num, nom_client, nom_adv, num_ibt, tribunal, accu, bureau_thk, result, commentaire, num))
		mydb.commit()
		clear()
	else:
		return True
...
def add_new():
	...
	cursor.execute("SELECT * FROM tbl_tahkik WHERE num = '"+num+"'")
	test = cursor.rowcount
	if (test == 1):
		messagebox.showerror("Erreur", "Ce dossier déja exist")
	else:
		query = "INSERT INTO tbl_tahkik (num, nom_client, nom_adv, num_ibt, tribunal, accu, bureau_thk, result, commentaire) Values(%s, %s, %s, %s, %s, %s, %s, %s, %s )"
		cursor.execute(query, (num, nom_client, nom_adv, num_ibt, tribunal, accu, bureau_thk, result, commentaire))
...
def delete_customer():
	...
	if messagebox.askyesno("Confirmation", "Confirmation de suppression de client"):
		query = "DELETE From tbl_tahkik WHERE num='"+id_0+"'"
		cursor.execute(query)
		mydb.commit()
		clear()
	else:
...
trv.heading(1,text="العدد")
trv.heading(2, text="المنوب")
trv.heading(3, text="الضد")
trv.heading(4, text="العدد الإبتدائي")
trv.heading(5, text="المحكمة")
trv.heading(6, text="دائرة الإتهام")
trv.heading(7, text="مكتب التحقيق")
trv.heading(8, text="المآل")
...
trv.column(9, width=120, minwidth=120)
...
lbl1 = Label(wrapper3, text="العدد")
...
ent3 = Entry(wrapper3, bd=3, width=30, textvariable=t3)
ent3.grid(row=2, column=1, padx=5, pady=3)

lbl4 = Label(wrapper3, text="العدد الإبتدائي")
lbl4.grid(row=3, column=0, padx=5, pady=3)
ent4 = Entry(wrapper3, bd=2, width=30, textvariable=t4)
ent4.grid(row=3, column=1, padx=5, pady=3)

lbl5 = Label(wrapper3, text="المحكمة")
...
lbl6 = Label(wrapper3, text="دائرة الإتهام")
lbl6.grid(row=5, column=0, padx=6, pady=3)
ent6 = Entry(wrapper3, bd=2, width=30, textvariable=t6)
ent6.grid(row=5, column=1, padx=5, pady=3)

lbl7 = Label(wrapper3, text="مكتب التحقيق")
...
lbl8 = Label(wrapper3, text="المآل")
